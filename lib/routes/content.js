...
const routes = application => {
  return new Promise(resolve => {
    ...
    */
    ...
    /*
     * @name Schedule - Schedule of Revisions to Sunrise/Sunset
     *
     * @param {object} req - HTTP Request
     * @param {object} res - HTTP Response
     * @param {object} next - Express callback
     */
    app.get(`/${config.content.base}/:type/:id/${config.content.actions.schedule}`, (req, res, next) => {
      let live = [];
      let scheduled = [];
      ...
      return utils.fill(req.content.types, req.content.type, references, database).then(ct => {
        ...
        const table = `content-type--${req.content.type.id}`;

        scheduled = todo;

        return database(table).select('*').where({
          id: req.params.id
        }).orderBy('revision', 'desc').limit(1);
      }).then(items => {
        const latest = utils.routes.identifier(items, req.content.type)[0];

        res.render('content/schedule', {
          live,
          scheduled,
          latest,
          config,
    ...
    // utils.fill(req.content.types, req.content.type, references, database)
